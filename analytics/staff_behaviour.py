...
def create_ui():
    ...
    with gr.Column(): # Use a Column or Row to group elements within the tab
        ...
        gr.Markdown(STAFF_BEHAVIOUR_PROMPT.split("## ")[0].split("purely to what is clearly visible in the video:\n\n")[-1])
        ...
        for key in output_keys:
            with gr.Group():
                ...
                output_textboxes[key] = gr.Textbox(label=f"", interactive=False, lines=5, autoscroll=True, elem_id=f"staff_behaviour_{key}")
...
def analyze_staffbehaviour_video(video_path):
    ...
    try:
        ...
        data = None
        possible_keys = ["BehaviorAndCompliance", "staff_behaviour", "Behaviour", "BehaviourAnalysis", "analysis", "results"]
