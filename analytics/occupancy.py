    ...
    with gr.Column(): # Use a Column or Row to group elements within the tab
        ...
        for key in output_keys:
            with gr.Group():
                ...
                output_textboxes[key] = gr.Textbox(label=f"Generated Output for {key.replace('_', ' ').title()}", interactive=False, lines=5, autoscroll=True, elem_id=f"occupancy_{key}")
...
def create_tab(video_player):
    with gr.Blocks() as occupancy_tab:
        ...
        gr.Markdown(OCCUPANCY_PROMPT) # Display the fixed prompt content

        gr.Markdown("## Analysis Result")
        with gr.Box():
            # This will hold the structured UI from occupancy_ui.py
            occupancy_output_display = gr.Blocks() # Use gr.Blocks to hold the dynamic UI

        analyze_button.click(
            analyze_occupancy_video,
            inputs=[video_player],
            outputs=occupancy_output_display,
